...
#from slam_gmapping import GMapping
...
class HRP_Teleop(object):
  ...
  def init(self):
    ...
    # self.distance_sensor_1 = 0
    # self.distance_sensor_left = 0
    # self.distance_sensor_right = 0
    ...
    self.pub_sonar = rospy.Publisher('sensor_1_topic', Range, queue_size=10)
    self.pub_sonar2 = rospy.Publisher('sensor_left_topic', Range, queue_size=10)
    self.pub_sonar3 = rospy.Publisher('sensor_right_topic', Range, queue_size=10)
    ...
    rospy.Subscriber('sensor_1_topic', Range, self.sonar1_callback)
    rospy.Subscriber('sensor_left_topic', Range, self.sonarLeft_callback)
    rospy.Subscriber('sensor_right_topic', Range, self.sonarRight_callback)
    
    rospy.Subscriber('/laser/scan', LaserScan, self.lidar_callback)
  ...
  def lidar_callback(self,msg):
    msg = '[Lidar] minimum distance is:' + str(msg.range_min)
    self.loginfo(msg)


  def odometry_callback(self,msg):
    msg = 'X: ' +  str(msg.pose.pose.position.x) + ',' + 'Y: ' + str(msg.pose.pose.position.y)
    self.loginfo(msg)
  ...
  def sonar1_callback(self, data):
      self.distance_sensor_1 = data.range
      msg = 'Sonar 1 distance:' + str(data.range) + 'm'
      self.loginfo(msg)
      

  def sonarLeft_callback(self, data):
      self.distance_sensor_left = data.range
      #self.loginfo('Sonar 2 distance: %f m', data.range)
      msg = 'Sonar 2 distance:' + str(data.range) + 'm'
      self.loginfo(msg)

  def sonarRight_callback(self, data):
      self.distance_sensor_right = data.range
      #self.loginfo('Sonar 3 distance: %f m', data.range)
      msg = 'Sonar 3 distance:' + str(data.range) + 'm'
      self.loginfo(msg)
  ...
  def update(self):
    ...
    # range.field_of_view = 1.5  # In radians
    # range.min_range = 0.2  # In meters
    # range.max_range = 10.0  # In meters
    self.pub_sonar.publish(range)
    self.pub_sonar2.publish(range)
    self.pub_sonar3.publish(range)
...
if __name__ == '__main__':
  ...
  rospy.init_node('HRP_keyboard_teleop', anonymous=True)
